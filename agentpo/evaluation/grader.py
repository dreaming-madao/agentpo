...
def math_equal(
    ...
    # if "aime" in data_name.lower():
    #     ret_score=boxed_reward_fn2(prediction,reference)[1]
    #     is_correct=True if ret_score==1.0 else False
    #     return is_correct
...
def is_equal(model_output: str, ground_truth: str, timeout_score: float = 0) -> bool:
    ...
    res=True if ret_score==1 else False
    return res


def math_equal_process(param):
    # try:
    #     is_correct=is_equal(param[-2], param[-1])
    # except:
    #     is_correct=math_equal(param[-2], param[-1])
    is_correct=math_equal(param[1], param[2], data_name=param[3])
    return is_correct
