...
def extract_answer(passage: str) -> str:
    ...
    pattern = r'''\\boxed{((?>[^{}]+|\{(?1)\})*)}'''
    match = regex.findall(pattern, passage, flags=regex.VERBOSE | regex.DOTALL)
    if match:
        pred = match[-1].strip()
    else:
        answer_pattern= r"(?i)Answer\s*:\s*([^\n]+)"
        match = re.findall(answer_pattern,passage)
        pred = match[-1] if match else "[INVALID]"
...
def boxed_reward_fn2(model_response, gt_answer, fast=False):
    try:
        ...
        ground_truth_boxed = "\\boxed{" + gt_answer + "}" if "\\boxed{" not in gt_answer else gt_answer
        ret_score, _ = verify_func([ground_truth_boxed], [model_response])
        ret_score = 1.0 if ret_score else 0.0
        result={"formatted": True}, ret_score

    except:
        result=boxed_reward_fn(model_response, gt_answer, fast=False)   
    return result
