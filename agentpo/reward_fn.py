...
def is_correct_minerva(solution_str: str, gt: str, gt_need_extract: bool = False, answer_pattern: str = r"(?i)Answer\s*:\s*([^\n]+)") -> tuple[bool, str]:
    ...
    pattern = r'''\\boxed{((?>[^{}]+|\{(?1)\})*)}'''
    match = regex.findall(pattern, solution_str, flags=regex.VERBOSE | regex.DOTALL)
    if match:
        pred = match[-1].strip()
    else:
...
if __name__ == "__main__":
    solution_str="Thus, the final answer is: \\boxed{(3.0, \\frac{\pi}{2})}"
    ground_truth= "\left( 3, \\frac{\pi}{2} \\right)"
    correct, pred = verify(solution_str, ground_truth, strict_box_verify=False, pause_tokens_index=None)
    print(correct,pred)
